...
### Filesystem Tools
- `read_file` - Read epic plan files (e.g., `.plans/phase-one.md`, `.cursor/plans/phase-one.md`)
  - Parameters: `target_file` = path to plan file
  - Use to read plan documents when creating epics from files
- `list_dir` - Explore directory structure to find plan files
  - Parameters: `target_directory` = directory to explore (e.g., `.plans/`, `.cursor/plans/`)
